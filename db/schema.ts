...
export type InsertMemobook = typeof memobooks.$inferInsert;

export const memos = pgTable("memos", {
    id: text('id').primaryKey().default(sql`gen_random_uuid()`),
    title: text('title').notNull(),
    content: jsonb('content').notNull(),
    memobookId: text('memobook_id').notNull().references(() => memobooks.id, { onDelete: 'cascade' }),
    createdAt: timestamp('created_at').$defaultFn(() => /* @__PURE__ */ new Date()),
    updatedAt: timestamp('updated_at').$defaultFn(() => /* @__PURE__ */ new Date())
});
...
export const schema = { user, session, account, verification, memobooks, memos, memobookRelations, memoRelations };